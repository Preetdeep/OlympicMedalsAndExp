...
---
\begin{abstract}
Using a data of over 120 years of Olympic history, I find that the mean age of medal winners is higher than that of non-medal winners. This relationship is more pronounced in the case of Gold medalists. I hypothesize that Age is a proxy for experience of Olympic Games and use show that the probability of getting a medal in the second appearance is higher than that in the first one. This could be due to experience, better handling of pressure, and better preparation. Our results are robust to change in models, controls, proxies, and years.
\end{abstract}
...
a<-a+geom_line(aes(colour=Type))+ylab("Mean Age")+labs(title = "Mean age of Medalists and non-medalists") 
print(a)
```
...
stargazer(reg.data, title="GLM Regression Results", column.labels=c("Whether Medal"),no.space=TRUE,float=FALSE,omit = "Sex",  style="qje", add.lines = list(c("Gender Controls", "Yes")), header=FALSE)
#summary(reg.data)
```
...
stargazer(reg.data, title="GLM Regression Results on Whether Gold Medal", column.labels=c("Whether Gold Medal"),no.space=TRUE,float=FALSE,omit = "Sex",  style="qje", add.lines = list(c("Gender Controls", "Yes")), header=FALSE)
#summary(reg.data) 
...
ADD MODEL
Our model is $Gold Gold= \Beta_1 Count2+Age+ Height+ Weight+Sex +Year+Sport$
...
athlete_events2$Year<-as.factor(athlete_events2$Year)
    ...
    stargazer(reg.data, title="GLM Regression Results for Gold Medal",no.space=TRUE,float=FALSE,omit = c("Sex","Year","Sport"),omit.labels = c("Control for Sex","Control for Year","Control for Sport"),  style="qje", header=FALSE, covariate.labels = c("Appearances", "Age", "Height",
                               "Weight"))
#    summary(reg.data)
```
...
```{r regappear, echo=FALSE,results='asis'}
    ...
    reg.data<-try(glm(model,data=athlete_events2, na.action = na.exclude ),silent=TRUE) #Data has to be Count in front of Athlete events
       ...
       stargazer(reg.data, title="GLM Regression Results for Medal", column.labels=c("Whether Medal"),no.space=TRUE,float=FALSE,omit = c("Sex","Year","Sport"),omit.labels = c("Control for Sex","Control for Year","Control for Sport"),  style="qje", header=FALSE, covariate.labels = c("Appearances", "Age", "Height",
                               "Weight"))
```
...
```{r regappeartests, echo=FALSE,results='asis'}
   ...
   #summary(reg.data1)
    ...
    #summary(reg.data3)
        ...
        stargazer(reg.data1,reg.data2,reg.data3, title="GLM Regression Results for Gold Medal", column.labels=c("Without Year and Sport","With Year","With Sport"),no.space=TRUE,float=FALSE,omit = c("Sex","Year","Sport"),omit.labels = c("Control for Sex","Control for Year","Control for Sport"),  style="qje", header=FALSE)
...
athlete_events2recent$Year<-as.factor(athlete_events2recent$Year)
    ...
    model<-(Medcod~ Count2+Age+ Height+ Weight+Sex +as.factor(Year)+Sport)#Count is the one that counts unique years
    reg.data<-try(glm(model,data=athlete_events2recent, na.action = na.exclude ),silent=TRUE) #Data has to be Count in front of Athlete events
    
     stargazer(reg.data, title="GLM Regression Results for Medal after 1950", column.labels=c("Whether Medal"),no.space=TRUE,float=FALSE,omit = c("Sex","Year","Sport"),omit.labels = c("Sex","Year","Sport"),  style="qje", header=FALSE, covariate.labels = c("Appearances", "Age", "Height",
                               "Weight"))
```
