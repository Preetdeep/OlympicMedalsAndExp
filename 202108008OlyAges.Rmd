...
CHECK THIS REGRESSION
```{r regappear, echo=FALSE}
...
athlete_events2$Count2<- ifelse(is.na(athlete_events2$Count),1,athlete_events2$Count)
    ...
    model<-(Gold~ Count2+Age+ Height+ Weight+Sex +as.factor(Year)+Sport)#Count is the one that counts unique years
    ...
    summary(reg.data)
```
We see from the regression that Age is now negative and Count is positive. It is statistically significant at ###0.03
